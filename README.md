...
1. **Cluster controller** (in `scaleDown`): when it sees `desired == 0 && len(members) == 1`, it (a) latches the surviving member's name in `status.dormantMember` so resurrection knows what to recreate, (b) stamps the annotation `etcd.lllamnyp.su/pause=true` onto the victim `EtcdMember`, and (c) deletes the `EtcdMember` CR.
2. **Member controller** (in the finalizer for that CR): notices the `etcd.lllamnyp.su/pause` annotation on the member it is processing and treats the deletion as a pause: re-parents the data PVC (`data-<name>`) so the EtcdCluster becomes its sole owner-controller, and skips `MemberRemove` so etcd's local data dir is left intact. The pause signal is on the member CR (not on cluster status) deliberately — controller-runtime caches the EtcdCluster and EtcdMember informers independently, so a `cluster.Status.DormantMember` read could race with the watch delivery of the Delete event and silently take the normal-removal path on a stale cache. The annotation on the member is read from the same object whose Delete event triggered the reconcile, so it is guaranteed cache-coherent. The annotation is *not* set on intermediate steps (3→2, 2→1) — only on the actual 1→0 step — so multi-member N→0 descents still call `MemberRemove` normally. See `TestHandleDeletion_IntermediateScaleDownStillCallsMemberRemove` and `TestHandleDeletion_PauseSurvivesStaleClusterCache` for the regression guards.
