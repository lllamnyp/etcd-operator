...
- **Apiserver-enforced validation**: CEL rules on the CRD (k8s 1.28+) reject `replicas: 0` with `storageMedium: Memory`, `storage: 0` with `storageMedium: Memory`, `storageMedium` changes after creation, and `storage` shrinks. No webhook / cert-manager dependency.
