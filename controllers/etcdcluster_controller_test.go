...
// TestBrokenMembers_StaysZeroAcrossMemoryPodLoss pins the contract
// documented in docs/concepts.md after the v1alpha2 redesign: the
// `Status.BrokenMembers` field does NOT observably surface during a
// memory pod-loss + auto-replacement cycle. The member controller
// detects the loss and self-deletes the EtcdMember in the same
// reconcile pass; by the time the cluster controller's updateStatus
// computes the count over the running set, the lost member is already
// Terminating and filtered out.
//
// This test exists to lock in that behaviour. If a future change adds
// an explicit Status update writing PodName="" before Delete (the
// approach a previous review proposal suggested to make a brief
// observable window real), this test will fail — at which point the
// docs and the predicate's contract should be revisited together. The
// inverse case (predicate accidentally triggers on a non-lost member)
// is covered by TestIsBroken_MemoryMemberWithLostPodIsBroken.
func TestBrokenMembers_StaysZeroAcrossMemoryPodLoss(t *testing.T) {
	ctx := context.Background()
	tru := true
	...
	healthyMember := func(name, podUID string) *lll.EtcdMember {
		return &lll.EtcdMember{
			ObjectMeta: metav1.ObjectMeta{
				Name: name, Namespace: "ns", UID: types.UID(name + "-uid"),
				Labels:     memberLabels("test", name),
			...
			Spec: lll.EtcdMemberSpec{
				...
				Storage:        quickQty(t, "128Mi"),
				StorageMedium:  lll.StorageMediumMemory,
				InitialCluster: buildInitialCluster([]string{"m-a", "m-b", "m-c"}, "test", "ns"),
				ClusterToken:   "ns-test-x",
				Bootstrap:      name == "m-a",
			},
			Status: lll.EtcdMemberStatus{
				PodName:  name,
				PodUID:   podUID,
				MemberID: fmt.Sprintf("%016x", uint64(len(name))),
				Conditions: []metav1.Condition{{
					Type: lll.MemberReady, Status: metav1.ConditionTrue, Reason: "PodReady",
					LastTransitionTime: metav1.Now(),
				}},
	...
	healthyPod := func(name, uid string) *corev1.Pod {
		return &corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name: name, Namespace: "ns", UID: types.UID(uid),
				OwnerReferences: []metav1.OwnerReference{{
					APIVersion: "lllamnyp.su/v1alpha2", Kind: "EtcdMember",
					Name: name, UID: types.UID(name + "-uid"), Controller: &tru, BlockOwnerDeletion: &tru,
				}},
			},
	...
	memberA := healthyMember("m-a", "uid-a")
	memberB := healthyMember("m-b", "uid-b")
	memberC := healthyMember("m-c", "uid-c") // its Pod is the one that will be missing
	podA := healthyPod("m-a", "uid-a")
	podB := healthyPod("m-b", "uid-b")
	// No podC — that's the loss condition.

	c, _ := newTestClient(t, cluster, memberA, memberB, memberC, podA, podB)
	fe := newFakeEtcd(0xdeadbeef,
		&etcdserverpb.Member{ID: 0x1, Name: "m-a", PeerURLs: []string{peerURL("m-a", "test", "ns")}},
		&etcdserverpb.Member{ID: 0x2, Name: "m-b", PeerURLs: []string{peerURL("m-b", "test", "ns")}},
		&etcdserverpb.Member{ID: 0x3, Name: "m-c", PeerURLs: []string{peerURL("m-c", "test", "ns")}},
	)

	cr := &EtcdClusterReconciler{Client: c, Scheme: testScheme(t), EtcdClientFactory: factoryReturning(fe)}
	mr := &EtcdMemberReconciler{Client: c, Scheme: testScheme(t), EtcdClientFactory: factoryReturning(fe)}

	// Phase 1: cluster controller reconciles BEFORE the member controller
	// has acted on the lost Pod. All three members are still in the
	// running set (no DeletionTimestamp); isBroken evaluates each.
	// Because Status.PodName has not been cleared on member C, the
	// predicate returns false — exactly as documented.
	if _, err := cr.Reconcile(ctx, ctrl.Request{NamespacedName: types.NamespacedName{Name: "test", Namespace: "ns"}}); err != nil {
		t.Fatalf("cluster reconcile (phase 1): %v", err)
	}
	gotCluster := mustGet(t, c, "test", "ns", &lll.EtcdCluster{})
	if gotCluster.Status.BrokenMembers != 0 {
		t.Fatalf("phase 1 BrokenMembers = %d, want 0 (predicate must not surface before PodName is cleared)",
			gotCluster.Status.BrokenMembers)
	}

	// Phase 2: member controller reconciles the lost member. It
	// observes podUID is set but the Pod is gone, and self-deletes
	// (DeletionTimestamp set, finalizer pending).
	if _, err := mr.Reconcile(ctx, ctrl.Request{NamespacedName: types.NamespacedName{Name: "m-c", Namespace: "ns"}}); err != nil {
		t.Fatalf("member reconcile of m-c (phase 2): %v", err)
	}
	gotC := &lll.EtcdMember{}
	err := c.Get(ctx, types.NamespacedName{Name: "m-c", Namespace: "ns"}, gotC)
	switch {
	case apierrors.IsNotFound(err):
		// finalizer already ran; fine.
	case err != nil:
		t.Fatalf("Get m-c: %v", err)
	case gotC.DeletionTimestamp.IsZero():
		t.Fatalf("phase 2: m-c must be marked for deletion (loss path)")
	}

	// Phase 3: cluster controller reconciles AFTER member C is dying.
	// `running` now excludes m-c (DeletionTimestamp set), so isBroken is
	// not consulted for it. BrokenMembers must remain 0.
	if _, err := cr.Reconcile(ctx, ctrl.Request{NamespacedName: types.NamespacedName{Name: "test", Namespace: "ns"}}); err != nil {
		t.Fatalf("cluster reconcile (phase 3): %v", err)
	}
	gotCluster = mustGet(t, c, "test", "ns", &lll.EtcdCluster{})
	if gotCluster.Status.BrokenMembers != 0 {
		t.Fatalf("phase 3 BrokenMembers = %d, want 0 (lost member is Terminating; filterRunningMembers excludes it)",
			gotCluster.Status.BrokenMembers)
	}
}
